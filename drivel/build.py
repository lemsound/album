...
# --- アルバム名の大きさ ---
# 数字だけ書けば rem 単位（おおよそ「文字の高さ何個ぶん」の感覚）。
# 目安: 3 = 控えめ / 4.4 = 標準 / 6 = 特大。小数もOK。
# スマホでは自動で小さく収まるよう調整されます。
アルバム名サイズ = 4.4

# --- スマホでのアルバム名の大きさ（任意）---
# 指定すると、スマホ表示だけ別サイズにできます。数字だけで rem 単位。
# 省略すると、上の「アルバム名サイズ」から自動で画面に収まる大きさになります。
スマホのアルバム名サイズ = 2.6

# --- 歌詞の文字の大きさ ---
# 数字だけ書けば rem 単位。目安: 1.0 = 標準 / 1.2 = 大きめ / 0.9 = 小さめ。
歌詞サイズ = 1.1
